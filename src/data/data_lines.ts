// Line based data like powerlines and fences
// Graciously joinked from https://github.com/Questwalker/votv-map

import type { Line } from "./types";

export function genLineId(line: Line, index: number): string {
    return `${line.Name ?? "line"}-${index}`;
}

export const lines: Line[] = [
    {
        Name: "Outer Fence 1",
        Color: "black",
        LineThickness: 2,
        Coordinates: [[-690, -700], [700, -700], [700, -215], [700, -200], [700, 700], [-700, 700], [-700, -70]]
    },
    {
        Name: "Outer Fence 2",
        Color: "black",
        LineThickness: 2,
        Coordinates: [[-700, -85], [-700, -700]]
    },
    {
        Name: "Stonehenge circle",
        Color: "black",
        LineThickness: 2,
        Coordinates: [[224, 476.5], [229.9, 478], [235.5, 480.5], [241.2, 482.3], [251, 489.5], [254.9, 494.3], [261.6, 504.4], [266.8, 515.4], [270, 526.9], [276, 526.9], [284.5, 535.7], [279.3, 546.4], [277.7, 552.5], [279.6, 558.2], [279.2, 564.2], [280.4, 570.2], [278.7, 576], [274, 580], [262.3, 582.5], [258, 586.8], [250.9, 596.7], [246.2, 600.6], [238.1, 609.7], [234.7, 615], [229, 617.2], [223, 617.6], [211.7, 613.5], [205.8, 612.6], [194.5, 608], [188.5, 608.3], [188, 602.4], [183.3, 598.5], [172.6, 593.4], [169.2, 588.6], [164.6, 584.7], [159.5, 581.8], [152.1, 572.6], [149, 561], [146.8, 555.4], [143.8, 550.4], [142.1, 544.6], [142.1, 538.8], [143.7, 526.8], [145.1, 521], [147.1, 515.6], [153.5, 505.4], [157.9, 501.2], [159.8, 495.6], [163.7, 491], [168.5, 487.5], [172.7, 482.9], [178.5, 483.7], [183.8, 486.4], [189.8, 485.7], [201, 481], [206.3, 478], [218, 476], [224, 476.5]]
    },
    { 
        Name: "Between-Mountain Cave",
        Color: "#907272",
        LineThickness: 2,
        Category: "Miscellaneous",
        Coordinates: [[21, -599], [212, -408.9]]
    },
    { 
        Name: "TR_1",
        Color: "#4b4d4e",
        LineThickness: 2,
        Category: "Transformers",
        Coordinates: [[64, 64.4], [71.4, 71.05], [173.9, 51.95], [280.35, 67.2], [331.5, 144.3], [387.5, 194.34], [400.9, 200.14]]
    },
    { 
        Name: "TR_2",
        Color: "#4b4d4e",
        LineThickness: 2,
        Category: "Transformers",
        Coordinates: [[64, 64.4], [49.3, 70.3], [-92.5, 88.1], [-216.7, 168.2], [-323.05, 254.8], [-427, 275.65], [-531.7, 253.65], [-541.4, 233.9]]
    },
    { 
        Name: "TR_3",
        Color: "#4b4d4e",
        LineThickness: 2,
        Category: "Transformers",
        Coordinates: [[64, 64.4], [64.06, 51.37], [119.36, -41.32], [152.2, -132.7], [155.8, -250.14], [40.77, -291.93], [-62, -303.9], [-177.43, -241.77], [-199.24, -333.3], [-353.04, -438.92], [-374.1, -462.6], [-391.88, -479.96]]
    },
    { 
        Name: "Corner Fence",
        Color: "black",
        LineThickness: 2,
        Coordinates: [[700, -331.4], [695.9, -340.1], [693.5, -342.8], [690.7, -344.5], [680.3, -344.8], [675, -346.3], [659.8, -368.6], [652.6, -383], [645.6, -401.6], [638.9, -422.5], [633.7, -431.1], [623.9, -441.3], [614.7, -451.3], [607.1, -462.4], [599.7, -477.9], [591, -509.2], [589.2, -544.8], [587.7, -558.4], [581.5, -575.6], [572.4, -588.5], [570, -593], [568, -599.8], [565.3, -616.7], [562.9, -643.5], [562.5, -671.7], [563.5, -680.6], [565.3, -687], [589, -700]]
    },
    { 
        Name: "World border",
        Color: "#5c5c5c88",
        LineThickness: 1,
        Coordinates: [[875, 875], [-875, 875], [-875, -875], [875, -875], [875, 875]]
    },
    
    { 
        // Small island
        Name: "Ritual Area",
        Color: "maroon",
        LineThickness: 2,
        Fill: "maroon",
        Coordinates: [[2012.25, 2000], [2010, 1997.5], [2007.5, 2000], [2010, 2002.5], [2012.25, 2000]]
    },
    { 
        Name: "Bridge",
        Color: "maroon",
        LineThickness: 2,
        Fill: "maroon",
        Coordinates: [[2007.8, 1999], [1999.2, 1999], [1999.2, 2001], [2007.8, 2001], [2007.8, 1999]]
    },
    { 
        Name: "Large island",
        Color: "maroon",
        LineThickness: 2,
        Fill: "maroon",
        Coordinates: [[1999.8, 2000], [1995, 1994.6], [1990.2, 2000], [1995, 2005.4], [1999.8, 2000]]
    }
]
